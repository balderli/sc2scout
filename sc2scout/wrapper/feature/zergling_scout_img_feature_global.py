...
class ZerglingScoutImgFeatureGlobal(ImgFeatExtractor):
    def __init__(self, compress_width, reverse):
        super(ZerglingScoutImgFeatureGlobal, self).__init__(compress_width, reverse)
        self._channel_num = GLOBAL_CHANNEL

    def reset(self, env):
        super(ZerglingScoutImgFeatureGlobal, self).reset(env)
        print('ZerglingScoutImgFeatureGlobal reset')
    ...
    def extract(self, env, obs):
        ...
        self.height_map_channel(env, image, channel_base)
    ...
    def height_map_channel(self, env, image, channel_base):
        height_map = env.unwrapped._height_feature
        if height_map is not None:
            image[:, :, channel_base] = height_map

        #self.save_map(image[:, :, channel_base], 'tmp.bmp')
